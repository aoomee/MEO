...
find_db_via_process() {
    # 跑在宿主机:看 mmwx 进程
    local pids
    pids=$(pgrep -f '(^|/)mmwx($|[[:space:]])' 2>/dev/null | head -3 || true)
    for pid in $pids; do
        local cwd
...
find_db_via_docker() {
    ...
    # 找运行中的 mmwx 容器
    local cids
    cids=$(docker ps --filter "ancestor=mmwx" --filter "name=mmwx" --format '{{.ID}}' 2>/dev/null | head -3 || true)
    if [[ -z "$cids" ]]; then
        # 兜底:任何 image 含 mmwx 的容器
        cids=$(docker ps --format '{{.ID}} {{.Image}}' 2>/dev/null | grep -i mmwx | awk '{print $1}' | head -3 || true)
    fi
    for cid in $cids; do
        ...
        while IFS= read -r line; do
            ...
            for cand in "$src/data/mmwx.db" "$src/mmwx.db" "$src/mmwx/data/mmwx.db"; do
                if [[ -f "$cand" ]]; then echo "$cand"; return 0; fi
            done
            # 如果 dst 是某个特定路径(/app, /etc/mmwx),按容器结构推
            case "$dst" in
                /etc/mmwx|/etc/mmwx/data|/data|/app|/app/data)
                    for cand in "$src/mmwx.db" "$src/data/mmwx.db"; do
                        if [[ -f "$cand" ]]; then echo "$cand"; return 0; fi
                    done
                    ;;
            esac
...
find_db_via_systemd() {
    ...
    if [[ -n "$workdir" && "$workdir" != "/" ]]; then
        for cand in "$workdir/data/mmwx.db" "$workdir/mmwx.db"; do
            if [[ -f "$cand" ]]; then echo "$cand"; return 0; fi
        done
    fi
    return 1
...
find_db_via_scan() {
    # 全盘扫描兜底 — 只搜常见路径以免无限慢
    local found
    found=$(find /etc /opt /root /var /srv /home -maxdepth 5 -name 'mmwx.db' 2>/dev/null | head -3 || true)
...
if [[ -z "$DB_PATH" ]]; then
    info "探测 mmwx 数据库路径..."
    for cand in /etc/mmwx/data/mmwx.db /etc/mmwx/mmwx.db; do
...
if [[ -z "$DB_PATH" || ! -f "$DB_PATH" ]]; then
    err "找不到 mmwx.db。可手动指定:bash $0 --db /path/to/mmwx.db"
    exit 1
fi
ok "数据库: $DB_PATH"
