...
func (s *Service) handleSub(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	var items []subItem
	for _, d := range subs.DefaultSubscriptions {
		items = append(items, subItem{d.Name + "(默认)", d.Description, d.CombinedCode})
	}
	if len(subs.DefaultSubscriptions) == 0 && subs.DefaultSubscription != nil {
	...
	// 未绑定套餐的管理员保持既有行为：展示订阅管理中排序第一的订阅。
	// 一旦绑定套餐，上面的 DefaultSubscriptions 非空，不再使用该回退项。
	if len(subs.DefaultSubscriptions) == 0 && subs.DefaultSubscription == nil && info.Role == "admin" {
		if sv, subviewErr := s.client.GetAdminSubview(ctx, info.Username); subviewErr == nil && sv != nil && sv.Subscription != nil {
			d := sv.Subscription
			items = []subItem{{d.Name, d.Description, d.CombinedCode}}
		}
	}
	if len(items) == 0 {
		_, _ = b.SendMessage(ctx, &bot.SendMessageParams{
