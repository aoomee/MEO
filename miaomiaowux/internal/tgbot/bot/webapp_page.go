...
// MEO Mini App 使用与主面板一致的雾白、松绿和墨绿视觉语言。
const webAppHTML = `<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
<title>MEO · 我的面板</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
:root{
 --brand:#356b5a;--brand-soft:#e7efeb;--ok:#2f7d5c;--warn:#b24d4d;--unknown:#8b9892;--radius:.75rem;
 --bg:#f5f7f6;--text:#17211d;--card:#ffffff;--muted:#66736d;--border:#dce4e0;
}
html.dark{
 --brand:#7eae9c;--brand-soft:rgba(126,174,156,.14);
 --bg:#111815;--text:#edf3f0;--card:#18211d;--muted:#a4b2ac;--border:#2b3933;
}
...
.brand{font-size:18px;font-weight:650;letter-spacing:.03em;display:inline-flex;align-items:baseline;line-height:1}
...
<body>
<header>
  <img id="logo" class="logo" src="__BRAND_LOGO__" data-custom="__CUSTOM_BRAND_LOGO__" alt="MEO">
...
window.__copy=copy;
// 客户端选择(value=主控订阅 ?t= 参数,口径同MEO 前端)
...
function renderHome(d){
 ...
 h+='</div>';
 var pts=d.package_traffic||[],t=d.traffic;
 if(pts.length){
  pts.forEach(function(p){
   var used=Number(p.cycle_used)||0,limit=(Number(p.limit_gb)||0)*1073741824,pct=limit>0?Math.min(100,used/limit*100):0;
   var dl=p.days_left!=null?Number(p.days_left):null;
   h+='<div class="card"><div class="title">流量 · '+esc(p.package_name||"套餐")+(p.is_primary?' <span class="badge">主套餐</span>':'')+'</div>';
   h+='<div class="row" style="margin:0"><span class="big">'+hb(used)+'</span><span class="muted">'+(limit>0?'/ '+p.limit_gb+' GB':'不限量')+'</span></div>';
   if(limit>0)h+='<div class="bar"><i style="width:'+pct.toFixed(1)+'%"></i></div><div class="row" style="margin:6px 0 0"><span class="muted">已用 '+pct.toFixed(1)+'%</span>'+(dl!=null?'<span class="'+(dl<=7?'warn':'muted')+'">剩 '+dl+' 天</span>':'<span></span>')+'</div>';
   h+='<div class="row" style="margin:4px 0 0"><span class="muted">本周期</span><span class="muted">↑'+hb(p.cycle_up)+' ↓'+hb(p.cycle_down)+'</span></div></div>';
  });
 }else if(t){
...
function renderStatus(d){
 ...
 ns.forEach(function(n){var ic,lb,col;
  ...
  if(adminServers){
   ...
   h+='<button class="xsw '+(n.xray_running?"on":"")+'" role="switch" aria-checked="'+(n.xray_running?"true":"false")+'" aria-label="'+esc(n.name)+' Xray '+(n.xray_running?"停止":"启动")+'" '+((n.agent_online===false)?"disabled":"")+' onclick="__xrayToggle('+Number(n.id)+','+(n.xray_running?"true":"false")+',this)"></button></span>';
...
function renderUserList(){
 ...
 list.forEach(function(u,i){var hasPkg=!!u.package_id;
  ...
  h+='<div style="display:flex;gap:6px"><select id="pk-'+i+'" class="inp" style="margin-top:0;flex:1" onchange="__pkgSwitchChanged('+i+','+(u.package_id||0)+')">';
  ...
  if(hasPkg){h+='<div id="io-'+i+'" style="display:none"><label class="muted" style="display:flex;align-items:center;gap:6px;font-size:12px"><input id="ie-'+i+'" type="checkbox"> 继承原到期时间</label>';
   h+='<label class="muted" style="display:flex;align-items:center;gap:6px;font-size:12px"><input id="it-'+i+'" type="checkbox"> 继承本周期已用流量（到新套餐下个重置日归零）</label></div>';}
  h+='</div>';});
...
function __pkgSwitchChanged(i,current){var sel=document.getElementById("pk-"+i),box=document.getElementById("io-"+i);if(!box)return;var changed=(parseInt(sel?sel.value:"0",10)||0)!==current;box.style.display=changed?"block":"none";if(!changed){var ie=document.getElementById("ie-"+i),it=document.getElementById("it-"+i);if(ie)ie.checked=false;if(it)it.checked=false;}}
window.__pkgSwitchChanged=__pkgSwitchChanged;
function __assignPkg(username,i,btn){
 var sel=document.getElementById("pk-"+i);var pid=parseInt(sel?sel.value:"0",10)||0;
 var ie=document.getElementById("ie-"+i),it=document.getElementById("it-"+i);
 if(pid<=0){toast("请选择套餐");return;}
 btn.disabled=true;var old=btn.textContent;btn.textContent="...";
 fetch("/api/tg-webapp/admin/user-assign",{method:"POST",headers:{"Content-Type":"application/json","X-Telegram-Init-Data":window.__init},body:JSON.stringify({username:username,package_id:pid,inherit_expire_date:!!(ie&&ie.checked),inherit_traffic:!!(it&&it.checked)})})
  .then(function(r){return r.json().then(function(j){return{ok:r.ok,j:j};});})
  .then(function(res){btn.disabled=false;btn.textContent=old;
