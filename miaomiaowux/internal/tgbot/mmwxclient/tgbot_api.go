...
type UserSubscriptionsResp struct {
	...
	DefaultSubscriptions []Subscription `json:"default_subscriptions,omitempty"`
}
...
func (c *Client) AssignPackage(ctx context.Context, username string, packageID int64, inheritExpireDate, inheritTraffic bool) error {
	return c.post(ctx, "/api/admin/packages/assign",
		map[string]any{"username": username, "package_id": packageID, "inherit_expire_date": inheritExpireDate, "inherit_traffic": inheritTraffic}, nil)
}

// Branding 仅包含 Mini App 需要的公开品牌字段。
type Branding struct {
	BrandTitle string `json:"brand_title"`
	LogoURL    string `json:"logo_url"`
}

// GetBranding 取主控当前生效的品牌；未配置时字段为空，由 Mini App 使用内置资源。
