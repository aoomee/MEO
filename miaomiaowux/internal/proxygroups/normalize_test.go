...
func TestNormalizeConfigAppliesDefaults(t *testing.T) {
	...
	if cat.Emoji != "" || cat.Icon != "" {
		t.Fatalf("expected decorative fields to be empty, got emoji=%q icon=%q", cat.Emoji, cat.Icon)
	}
	if cat.RuleName != "ai" {
...
func TestNormalizeConfigSuppressesEmojiAndIcon(t *testing.T) {
	input := []byte(`[
		{
			"name": "only-emoji",
			"label": "仅 Emoji",
	...
	if got[0].Emoji != "" || got[0].Icon != "" {
		t.Fatalf("expected only-emoji item to be normalized without decoration, got emoji=%q icon=%q", got[0].Emoji, got[0].Icon)
	}
	if got[1].Emoji != "" || got[1].Icon != "" {
		t.Fatalf("expected only-icon item to be normalized without decoration, got emoji=%q icon=%q", got[1].Emoji, got[1].Icon)
	}
}

func TestNormalizeConfigMergesDuplicateCategoryAndGroup(t *testing.T) {
	input := []byte(`[
		{"name":"social","label":"社交媒体","site_rules":[{"key":"facebook"},{"key":"tiktok"}]},
		{"name":"social","label":"Tiktok","site_rules":[{"key":"tiktok"}]},
		{"name":"tiktok","label":"Tiktok","site_rules":[{"key":"tiktok"}]},
		{"name":"tiktok-copy","label":"Tiktok","site_rules":[{"key":"tiktok-extra"}]}
	]`)
