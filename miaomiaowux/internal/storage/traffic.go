...
const (
	...
	//   - wal_autocheckpoint=0 关闭每连接默认 1000 页自动 checkpoint，统一交给阈值化周期任务，
	//     避免高频更新相同页面时每几秒重复写回主库。
	//   - journal_size_limit=64MB  显式 TRUNCATE checkpoint 后把 -wal 文件截回 ≤64MB(默认 -1=无限)。
	//     周期巡检只做 PASSIVE checkpoint，让 SQLite 复用 WAL 空间而不反复截断文件。
	sqliteDSNPragma = "_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=synchronous(normal)&_pragma=wal_autocheckpoint(0)&_pragma=journal_size_limit(67108864)"
...
var (
	...
	// 该套餐已作为独立套餐绑定给此用户,不能再设为主套餐 —— 否则「已绑定套餐」里会出现
	// 两条完全相同的记录。
	ErrPackageAlreadyAssignedIndependently = errors.New("package is already assigned to this user as an independent package")
...
type Package struct {
	...
	// NodesConfigured 记录"过滤掉已删节点之前, nodes 是否非空"。
	//
	// 所有读路径都会用 filterAliveNodeIDs 把已删节点滤掉,于是"从未配置过节点"和
	// "配置过但节点全被删"在调用方看来都是空列表。而空列表的产品语义是"放行全部节点",
	// 这会让后者从"限定 N 个节点"静默升级成"不限节点" —— 越权发号。
	// 有了这个标记,下游就能把后者正确判定为"没有可用节点"。
	NodesConfigured         bool              `json:"nodes_configured"`
	...
	NodeNameOverrideEnabled bool              `json:"node_name_override_enabled"`    // 是否在订阅中启用套餐内节点名称
	...
	// 套餐内每个用户独立的 per-node 流量额度（GB）。缺省/0 = 不限制。
	NodeTrafficLimits map[int64]float64 `json:"node_traffic_limits,omitempty"`
	...
	TemplateFilename      string               `json:"template_filename"`       // 套餐绑定的 Clash 模板;空 = 走系统默认模板
	SurgeTemplateFilename string               `json:"surge_template_filename"` // 套餐绑定的 Surge 模板;空 = 走系统默认模板
...
type Node struct {
	...
	// 中转组:落地节点的 dialer-proxy 指向一个由多个中转节点组成的 url-test 代理组。RelayGroupName 是组名,
	// RelayGroupNodeIDs 是中转节点 ID 列表(按 ID 存,生成订阅时按 ID 取当前名,改名无影响)。
...
type SystemConfig struct {
	...
	EnableMiaomiaowuFeatures     bool   // 启用MEO功能（模板、订阅管理等菜单）
...
func NewTrafficRepositoryFromConfig(cfg DatabaseConfig) (*TrafficRepository, error) {
	...
	if cfg.Driver == "postgres" {
		...
		// pgx v5 默认 cache_describe 只缓存参数描述,预处理语句每次仍会发 Parse。
		// cache_statement 让同 SQL 真正命中连接级 prepared cache,对应 changelog
		// 「PostgreSQL 批量写入优化,预处理语句缓存真正命中」。
		values.Set("default_query_exec_mode", "cache_statement")
	...
	if cfg.Driver == "sqlite" {
		...
		if _, err := db.Exec("PRAGMA wal_autocheckpoint=0"); err != nil {
			log.Printf("[storage] disable wal_autocheckpoint failed (non-fatal): %v", err)
		}
	}
...
// CheckpointPassive 供在线周期巡检使用。PASSIVE 不等待正在读写数据库的连接，也不要求
// 排他锁和截断 WAL 文件；它只把当前能够安全写回的帧推进到主库，已 checkpoint 的 WAL
// 空间随后由 SQLite 原地复用。这样不会在小 WAL 上反复 TRUNCATE、改写主库并争抢锁。
//
// 返回 remaining 表示本轮后仍未能写回的帧数；活跃 reader 暂时钉住少量帧是正常现象。
// 备份前仍应调用 Checkpoint，它会显式执行 TRUNCATE 以取得完整的单文件快照。
func (r *TrafficRepository) CheckpointPassive() (remaining int, err error) {
	if r == nil || r.db == nil || r.config.Driver == "postgres" {
		return 0, nil
	}
	var busy, logFrames, checkpointed int
	if err = r.db.QueryRow("PRAGMA wal_checkpoint(PASSIVE)").Scan(&busy, &logFrames, &checkpointed); err != nil {
		return 0, fmt.Errorf("wal checkpoint passive: %w", err)
	}
	remaining = logFrames - checkpointed
	if remaining < 0 {
		remaining = 0
	}
...
`
	...
	// 中转组:落地节点的 dialer-proxy 指向多个中转节点组成的 url-test 组。成员按节点 ID 存(改名无影响)。
	...
	// ip_family:节点 IP 版本归属(取代"靠 clash server 含冒号"判定 v6)。回填:server 含冒号=v6,其余空=按 v4。
	...
	if _, err := r.db.Exec(`UPDATE nodes SET ip_family = 'v6'
		WHERE IFNULL(ip_family, '') = ''
		  ...
		  AND IFNULL(json_extract(clash_config, '$.server'), '') LIKE '%:%'`); err != nil {
		return fmt.Errorf("backfill ip_family: %w", err)
	}
...
`
	...
	// 用户个人默认模板文件名(套餐订阅优先用它,详见 loadTemplate)
...
`
	...
	// 套餐内节点名称总开关。已有别名的套餐自动保持启用，避免升级后订阅名称突变。
	if _, err := r.db.Exec("ALTER TABLE packages ADD COLUMN node_name_override_enabled INTEGER NOT NULL DEFAULT 0"); err == nil {
		_, _ = r.db.Exec("UPDATE packages SET node_name_override_enabled = 1 WHERE COALESCE(node_name_overrides, '{}') NOT IN ('', '{}')")
	}
...
`
	...
	if err := r.ensureRemoteServerColumn("domain_v6", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	...
	// 探针展示元数据。地域允许用户覆盖；续期周期仅用于顺延到期日。
	if err := r.ensureRemoteServerColumn("region", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	if err := r.ensureRemoteServerColumn("region_country", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	if err := r.ensureRemoteServerColumn("region_name", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	if err := r.ensureRemoteServerColumn("region_city", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	...
	_, _ = r.db.Exec("ALTER TABLE packages ADD COLUMN surge_template_filename TEXT NOT NULL DEFAULT ''")
	// 早期版本允许把 Surge .conf 写进通用模板字段，升级时归位，避免覆盖新的 Clash 选择。
	_, _ = r.db.Exec(`UPDATE packages
		SET surge_template_filename = template_filename, template_filename = ''
		WHERE surge_template_filename = '' AND LOWER(template_filename) LIKE '%.conf'`)
	...
	// 用户流量上限覆写(bytes)。刻意不给 DEFAULT — 存量行为 NULL 即"继承套餐"。
	_, _ = r.db.Exec("ALTER TABLE users ADD COLUMN traffic_limit_override INTEGER")
	...
	_, _ = r.db.Exec("ALTER TABLE packages ADD COLUMN node_traffic_limits TEXT DEFAULT '{}'")
	if _, err := r.db.Exec(`CREATE TABLE IF NOT EXISTS package_user_node_traffic_baselines (
		username TEXT NOT NULL,
		...
		return fmt.Errorf("migrate package node traffic baselines: %w", err)
	}
	if _, err := r.db.Exec(`CREATE TABLE IF NOT EXISTS package_node_traffic_suspensions (
		username TEXT NOT NULL,
		package_id INTEGER NOT NULL,
		node_id INTEGER NOT NULL,
		kind TEXT NOT NULL,
		...
		return fmt.Errorf("migrate package node traffic suspensions: %w", err)
	}
...
`
	...
	// SQLite 无法直接修改 CHECK，所以只在 SQLite 下读 sqlite_master 并重建旧表。
	// PostgreSQL 新表已使用当前约束，且绝不应访问 SQLite 系统表。
	if r.config.Driver != "postgres" {
		var checkSQL string
...
`
	...
	if err := r.migrateUserPackageAssignments(); err != nil {
		return err
	}
...
`
	...
	// 转发管理(转发组/链)相关表
	if err := r.ensureForwardTables(context.Background()); err != nil {
		return fmt.Errorf("migrate forward tables: %w", err)
	}
	if err := r.ensureWireGuardTables(context.Background()); err != nil {
...
func (r *TrafficRepository) UpdateUserCustomShortCode(ctx context.Context, username, code string) error {
	...
	var previous string
	if err := r.db.QueryRowContext(ctx, `SELECT COALESCE(custom_user_short_code, '') FROM user_tokens WHERE username = ?`, username).Scan(&previous); err != nil {
		return fmt.Errorf("read existing user custom short code: %w", err)
	}
...
type User struct {
	...
	Remark       string
	PackageID    int64
	// PackageAssignmentID is a transient selector used by multi-package
	// provisioning. Legacy callers leave it at zero and retain their original
	// users.package_id behavior.
	PackageAssignmentID int64 `json:"-"`
	IsReset             bool
	ResetDay            int
	...
	// TrafficLimitOverride 用户级流量上限覆写(bytes),优先于套餐的 traffic_limit_bytes。
	// 三态:nil = 继承套餐;0 = 显式不限流量;>0 = 覆写值。
	// 判断必须用"指针是否非 nil",不能用 value > 0 —— 0 是有意义的值。
...
func (r *TrafficRepository) GetUser(ctx context.Context, username string) (User, error) {
	...
	row := r.db.QueryRowContext(ctx, `SELECT username, password_hash, COALESCE(email, ''), COALESCE(nickname, ''), COALESCE(avatar_url, ''), COALESCE(role, ''), is_active, COALESCE(package_id, 0), COALESCE(is_reset, 0), COALESCE(reset_day, 1), last_reset_at, package_end_date, speed_limit_override, device_limit_override, traffic_limit_override, COALESCE(node_speed_limit_overrides, '{}'), COALESCE(node_device_limit_overrides, '{}'), COALESCE(totp_secret, ''), COALESCE(totp_enabled, 0), COALESCE(recovery_codes, '[]'), created_at, updated_at FROM users WHERE username = ? LIMIT 1`, username)
	...
	if err := row.Scan(&user.Username, &user.PasswordHash, &user.Email, &user.Nickname, &user.AvatarURL, &user.Role, &active, &user.PackageID, &isReset, &user.ResetDay, &lastResetAt, &endDate, &speedOverride, &deviceOverride, &trafficOverride, &nodeSpeedJSON, &nodeDeviceJSON, &user.TOTPSecret, &totpEnabled, &user.RecoveryCodes, &user.CreatedAt, &user.UpdatedAt); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *TrafficRepository) ListUsers(ctx context.Context, limit int) ([]User, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT username, password_hash, COALESCE(email, ''), COALESCE(nickname, ''), COALESCE(avatar_url, ''), COALESCE(role, ''), is_active, COALESCE(remark, ''), COALESCE(package_id, 0), COALESCE(is_reset, 0), COALESCE(reset_day, 1), last_reset_at, package_end_date, speed_limit_override, device_limit_override, traffic_limit_override, COALESCE(node_speed_limit_overrides, '{}'), COALESCE(node_device_limit_overrides, '{}'), created_at, updated_at FROM users ORDER BY created_at ASC LIMIT ?`, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		var trafficOverride sql.NullInt64
		var nodeSpeedJSON, nodeDeviceJSON string
		if err := rows.Scan(&user.Username, &user.PasswordHash, &user.Email, &user.Nickname, &user.AvatarURL, &user.Role, &active, &user.Remark, &user.PackageID, &isReset, &user.ResetDay, &lastResetAt, &endDate, &speedOverride, &deviceOverride, &trafficOverride, &nodeSpeedJSON, &nodeDeviceJSON, &user.CreatedAt, &user.UpdatedAt); err != nil {
...
func (r *TrafficRepository) DeleteUser(ctx context.Context, username string) error {
	...
	// 套餐关联按 username 记录,不随 users 级联。留下孤儿行会让该套餐之后
	// 编辑/删除时按 assignment.Username 反查用户失败("用户不存在"),整个套餐操作被卡死。
	_, err = tx.ExecContext(ctx, `DELETE FROM user_package_assignments WHERE username = ?`, username)
	if err != nil {
		return fmt.Errorf("delete user package assignments: %w", err)
	}
...
// ListMMWProxyProviderConfigs 返回所有MEO模式的代理集合配置
...
func (r *TrafficRepository) ListPackages(ctx context.Context) ([]Package, error) {
	...
	const query = `
		SELECT id, name, COALESCE(description, ''), traffic_limit_bytes, cycle_days,
		       ...
		       COALESCE(auto_speed_limit_json, ''), COALESCE(short_code, ''), COALESCE(traffic_mode, 'oneway'), COALESCE(template_filename, ''), COALESCE(surge_template_filename, ''), COALESCE(node_multipliers, '{}'), COALESCE(node_name_overrides, '{}'), COALESCE(node_name_override_enabled, 0), COALESCE(node_speed_limits, '{}'), COALESCE(node_device_limits, '{}'), COALESCE(node_traffic_limits, '{}'), created_at, updated_at
		FROM packages
	...
	for rows.Next() {
		var pkg Package
		var isReset, nodeNameOverrideEnabled int
		var nodesJSON, autoSpeedJSON, nodeMultJSON, nodeNamesJSON, nodeSpeedJSON, nodeDeviceJSON, nodeTrafficJSON string
		err := rows.Scan(&pkg.ID, &pkg.Name, &pkg.Description, &pkg.TrafficLimitBytes,
			...
			&autoSpeedJSON, &pkg.ShortCode, &pkg.TrafficMode, &pkg.TemplateFilename, &pkg.SurgeTemplateFilename, &nodeMultJSON, &nodeNamesJSON, &nodeNameOverrideEnabled, &nodeSpeedJSON, &nodeDeviceJSON, &nodeTrafficJSON, &pkg.CreatedAt, &pkg.UpdatedAt)
		if err != nil {
			return nil, fmt.Errorf("scan package: %w", err)
		}
		pkg.IsReset = isReset != 0
		pkg.NodeNameOverrideEnabled = nodeNameOverrideEnabled != 0
		...
		if nodeTrafficJSON != "" && nodeTrafficJSON != "{}" {
			unmarshalStringKeyedMap(nodeTrafficJSON, &pkg.NodeTrafficLimits)
		}
	...
	if len(idUnion) > 0 {
		if alive, err := r.aliveNodeIDs(ctx, idUnion); err == nil {
			for i := range packages {
				...
				packages[i].NodesConfigured = true
...
func (r *TrafficRepository) GetPackage(ctx context.Context, id int64) (*Package, error) {
	...
	err := r.db.QueryRowContext(ctx, query, id).Scan(&pkg.ID, &pkg.Name, &pkg.Description,
		...
		&pkg.TemplateFilename, &pkg.SurgeTemplateFilename, &nodeMultJSON, &nodeNamesJSON, &nodeNameOverrideEnabled, &nodeSpeedJSON, &nodeDeviceJSON, &nodeTrafficJSON, &pkg.CreatedAt, &pkg.UpdatedAt)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	pkg.NodesConfigured = len(pkg.Nodes) > 0
...
func (r *TrafficRepository) CreatePackage(ctx context.Context, pkg Package) (int64, error) {
	...
	nodeTrafficJSON := serializeNodeFloatMap(pkg.NodeTrafficLimits, pkg.Nodes)
	...
	const query = `
		INSERT INTO packages (name, description, traffic_limit_bytes, cycle_days, is_reset, reset_day, nodes, speed_limit_mbps, device_limit, auto_speed_limit_json, short_code, traffic_mode, template_filename, surge_template_filename, node_multipliers, node_name_overrides, node_name_override_enabled, node_speed_limits, node_device_limits, node_traffic_limits)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	...
	result, err := r.db.ExecContext(ctx, query, name, pkg.Description, pkg.TrafficLimitBytes,
		pkg.CycleDays, isReset, pkg.ResetDay, string(nodesJSON), pkg.SpeedLimitMbps, pkg.DeviceLimit, autoSpeedJSON, shortCode, trafficMode, pkg.TemplateFilename, pkg.SurgeTemplateFilename, nodeMultJSON, nodeNamesJSON, boolToInt(pkg.NodeNameOverrideEnabled), nodeSpeedJSON, nodeDeviceJSON, nodeTrafficJSON)
	if err != nil {
...
func (r *TrafficRepository) UpdatePackage(ctx context.Context, pkg Package) error {
	...
	const query = `
		...
		SET name = ?, description = ?, traffic_limit_bytes = ?, cycle_days = ?,
		    ...
		    auto_speed_limit_json = ?, traffic_mode = ?, template_filename = ?, surge_template_filename = ?, node_multipliers = ?, node_name_overrides = ?, node_name_override_enabled = ?,
		    node_speed_limits = ?, node_device_limits = ?, node_traffic_limits = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`
	...
	result, err := tx.ExecContext(ctx, query, name, pkg.Description, pkg.TrafficLimitBytes,
		pkg.CycleDays, isReset, pkg.ResetDay, string(nodesJSON), pkg.SpeedLimitMbps, pkg.DeviceLimit, autoSpeedJSON, trafficMode, pkg.TemplateFilename, pkg.SurgeTemplateFilename, nodeMultJSON, nodeNamesJSON, boolToInt(pkg.NodeNameOverrideEnabled), nodeSpeedJSON, nodeDeviceJSON, nodeTrafficJSON, pkg.ID)
	if err != nil {
	...
	if err := ensurePackageNodeTrafficBaselinesTx(ctx, tx, &pkg); err != nil {
		return fmt.Errorf("initialize new package node traffic limits: %w", err)
	}
	if err := tx.Commit(); err != nil {
...
// 将包分配给用户
// PackageAssignmentOptions controls the accounting boundary used when switching
// packages. The zero value keeps the historical behavior: a different package
// starts with zero used traffic. Expiry inheritance is resolved by the handler,
// because storage receives the already selected endDate.
type PackageAssignmentOptions struct {
	InheritTraffic bool
}
...
func (r *TrafficRepository) AssignPackageToUser(ctx context.Context, username string, packageID int64, startDate time.Time, endDate time.Time, isReset bool, resetDay int) error {
	return r.AssignPackageToUserWithOptions(ctx, username, packageID, startDate, endDate, isReset, resetDay, PackageAssignmentOptions{})
}

func (r *TrafficRepository) AssignPackageToUserWithOptions(ctx context.Context, username string, packageID int64, startDate time.Time, endDate time.Time, isReset bool, resetDay int, options PackageAssignmentOptions) error {
	if r == nil || r.db == nil {
		return errors.New("traffic repository not initialized")
	}
	...
	defer func() { _ = tx.Rollback() }()
	var primaryAssignmentID int64
	var primaryLegacy int
	primaryErr := tx.QueryRowContext(ctx, `SELECT id,COALESCE(legacy_source,0) FROM user_package_assignments WHERE username=? AND status='active' ORDER BY is_primary DESC,created_at,id LIMIT 1`, username).Scan(&primaryAssignmentID, &primaryLegacy)
	if primaryErr == nil && primaryAssignmentID > 0 && primaryLegacy == 0 {
		return errors.New("primary package is an independent assignment; update it through the package assignment service")
	}
	if primaryErr != nil && !errors.Is(primaryErr, sql.ErrNoRows) {
		return fmt.Errorf("read primary package assignment: %w", primaryErr)
	}

	// 同一用户不能对同一套餐既有主绑定又有独立绑定:两者在「已绑定套餐」「复制订阅链接」
	// 里显示为完全相同的两条,管理员无法分辨,退订也分不清该退哪个。
	//
	// 触发路径不是数据脏,而是 UI 状态串用:管理套餐弹窗里「新增为独立套餐」和底部
	// 「确认保存」共用同一个 selectedPackageId —— 管理员挨个点选套餐加成独立绑定后,
	// 高亮停在最后点的那个,再点「确认保存」就把主套餐也指到了它上面。
	// 前端已修正,这里再兜一道:任何入口都不允许造出这种重复。
	var duplicateIndependent int64
	if err := tx.QueryRowContext(ctx,
		`SELECT COUNT(1) FROM user_package_assignments WHERE username=? AND package_id=? AND status='active' AND COALESCE(legacy_source,0)=0`,
		username, packageID).Scan(&duplicateIndependent); err != nil {
		return fmt.Errorf("check duplicate package assignment: %w", err)
	}
	if duplicateIndependent > 0 {
		return ErrPackageAlreadyAssignedIndependently
	}
	...
	const query = `
		...
		SET traffic_limit_override = CASE WHEN package_id = ? THEN traffic_limit_override ELSE NULL END,
		    ...
		    is_reset = ?, reset_day = ?,
		    last_reset_at = CASE WHEN package_id = ? THEN last_reset_at ELSE CURRENT_TIMESTAMP END,
		    updated_at = CURRENT_TIMESTAMP
		WHERE username = ?
	`

	result, err := tx.ExecContext(ctx, query, packageID, packageID, startDate, endDate, packageID, endDate, isResetInt, resetDay, packageID, username)
	if err != nil {
	...
	if packageChanged {
		if options.InheritTraffic {
			// A carried cycle must still record a boundary for the new package.
			// Do this explicitly instead of relying on SET-expression evaluation
			// order, which differs between SQLite and PostgreSQL adapters.
			if _, err := tx.ExecContext(ctx, `UPDATE users SET last_reset_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE username=?`, username); err != nil {
				return fmt.Errorf("mark inherited package cycle: %w", err)
			}
		}
		if _, err := tx.ExecContext(ctx, `DELETE FROM package_node_traffic_suspensions WHERE username=?`, username); err != nil {
			return fmt.Errorf("clear old package node suspensions: %w", err)
		}
		if !options.InheritTraffic {
		...
		if err := replacePackageNodeTrafficBaselinesTx(ctx, tx, username, pkg); err != nil {
			return fmt.Errorf("initialize package node traffic limits: %w", err)
		}
	}
	var legacyAssignmentID int64
	legacyErr := tx.QueryRowContext(ctx, `SELECT id FROM user_package_assignments WHERE username=? AND legacy_source=1`, username).Scan(&legacyAssignmentID)
	if errors.Is(legacyErr, sql.ErrNoRows) {
		code, codeErr := generateAssignmentShortCode()
		if codeErr != nil {
			return codeErr
		}
		if _, legacyErr = tx.ExecContext(ctx, `UPDATE user_package_assignments SET is_primary=0,updated_at=CURRENT_TIMESTAMP WHERE username=?`, username); legacyErr != nil {
			return fmt.Errorf("demote previous primary package assignment: %w", legacyErr)
		}
		_, legacyErr = tx.ExecContext(ctx, `INSERT INTO user_package_assignments(username,package_id,package_start_date,package_end_date,is_reset,reset_day,last_reset_at,traffic_limit_override,status,is_primary,short_code,traffic_warned_80,over_limit_enforced,legacy_source)
			SELECT username,package_id,package_start_date,package_end_date,is_reset,reset_day,last_reset_at,traffic_limit_override,'active',1,?,traffic_warned_80,over_limit_enforced,1 FROM users WHERE username=?`, code, username)
	} else if legacyErr == nil {
		if _, legacyErr = tx.ExecContext(ctx, `UPDATE user_package_assignments SET is_primary=0,updated_at=CURRENT_TIMESTAMP WHERE username=? AND id<>?`, username, legacyAssignmentID); legacyErr != nil {
			return fmt.Errorf("demote previous primary package assignment: %w", legacyErr)
		}
		_, legacyErr = tx.ExecContext(ctx, `UPDATE user_package_assignments SET package_id=?,package_start_date=?,package_end_date=?,is_reset=?,reset_day=?,last_reset_at=(SELECT last_reset_at FROM users WHERE username=?),traffic_limit_override=(SELECT traffic_limit_override FROM users WHERE username=?),status='active',is_primary=1,traffic_warned_80=0,updated_at=CURRENT_TIMESTAMP WHERE id=?`, packageID, startDate, endDate, isReset, resetDay, username, username, legacyAssignmentID)
	}
	if legacyErr != nil {
		return fmt.Errorf("sync legacy package assignment: %w", legacyErr)
	}
	if err := tx.Commit(); err != nil {
...
func (r *TrafficRepository) removePackageFromUser(ctx context.Context, username string, preserveForRenewal bool) error {
	...
	var legacyAssignmentID int64
	_ = r.db.QueryRowContext(ctx, `SELECT id FROM user_package_assignments WHERE username=? AND legacy_source=1`, username).Scan(&legacyAssignmentID)
	...
	_, _ = r.db.ExecContext(ctx, `DELETE FROM package_user_node_traffic_baselines WHERE username=?`, username)
	_, _ = r.db.ExecContext(ctx, `DELETE FROM package_node_traffic_suspensions WHERE username=?`, username)
	if legacyAssignmentID > 0 {
		if err := r.DeleteUserPackageAssignment(ctx, legacyAssignmentID); err != nil && !errors.Is(err, ErrPackageNotFound) {
			return fmt.Errorf("remove legacy package assignment: %w", err)
		}
	}
...
func (r *TrafficRepository) ListPackageAssignmentSubaccountsByServerName(ctx context.Context, serverName string) ([]ActiveSubaccountForLimiter, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT sa.assignment_id,sa.username,sa.email,sa.routed_node_id,COALESCE(n.inbound_tag,''),sa.is_active
		FROM package_assignment_subaccounts sa
		INNER JOIN nodes n ON sa.routed_node_id=n.id
		WHERE n.original_server=? AND n.node_type='routed'
	`, serverName)
	if err != nil {
	...
	for rows.Next() {
		var a ActiveSubaccountForLimiter
		var active int
		if err := rows.Scan(&a.AssignmentID, &a.Username, &a.Email, &a.RoutedNodeID, &a.InboundTag, &active); err != nil {
			return nil, err
		}
		a.IsActive = active != 0
		if a.InboundTag != "" {
...
// UpdateUserTrafficLimitOverride 写用户级流量上限覆写(bytes)。
// nil = 清除覆写(继承套餐);0 = 显式不限流量;>0 = 覆写值。
// 刻意独立于 UpdateUserLimitOverrides:那个是无条件全量 UPDATE,把流量并进去会让
// 只改限速的调用方(限速弹窗 / MCP user_set_limits)把流量覆写静默清成 NULL。
func (r *TrafficRepository) UpdateUserTrafficLimitOverride(ctx context.Context, username string, limitBytes *int64) error {
	...
	_, _ = r.db.ExecContext(ctx, `UPDATE user_package_assignments SET traffic_limit_override=?,updated_at=CURRENT_TIMESTAMP WHERE username=? AND is_primary=1 AND status='active'`, limitBytes, username)
	return nil
}
...
func (r *TrafficRepository) UpdateUserOverLimitState(ctx context.Context, username string, isOverLimit, enforced bool) error {
	...
	_, err := r.db.ExecContext(ctx,
		...
		over, done, username)
	if err == nil {
		_, _ = r.db.ExecContext(ctx, `UPDATE user_package_assignments SET over_limit_enforced=?,updated_at=CURRENT_TIMESTAMP WHERE username=? AND is_primary=1 AND COALESCE(legacy_source,0)=1`, done, username)
	}
	return err
}
...
func (r *TrafficRepository) UpdateUserTrafficWarned80(ctx context.Context, username string, warned bool) error {
	...
	if err == nil {
		_, _ = r.db.ExecContext(ctx, `UPDATE user_package_assignments SET traffic_warned_80=?,updated_at=CURRENT_TIMESTAMP WHERE username=? AND is_primary=1 AND COALESCE(legacy_source,0)=1`, val, username)
	}
	return err
}
...
func (r *TrafficRepository) LicenseUsage(ctx context.Context) (servers, nodes, users int, err error) {
	...
	licensedNodes, countErr := r.CountLicensedNodes(ctx)
	if countErr != nil {
		return 0, 0, 0, countErr
	}
	nodes = int(licensedNodes)
...
func (r *TrafficRepository) ListRemoteServers(ctx context.Context) ([]RemoteServer, error) {
	...
	const query = `SELECT id, name, token, status, last_heartbeat, COALESCE(ip_address, ''), COALESCE(ip_address_v6, ''), COALESCE(ipv6_enabled, 1), COALESCE(offline_notified, 0), COALESCE(domain, ''), COALESCE(domain_v6, ''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&server.ID, &server.Name, &server.Token, &server.Status, &lastHeartbeat, &server.IPAddress, &server.IPAddressV6, &server.IPv6Enabled, &server.OfflineNotified, &server.Domain, &server.DomainV6,
	...
	if len(servers) > 0 {
		...
		type nodeSet struct {
			...
			includeStats                       bool
			min, max                           int
			systemRxCycle, systemTxCycle       int64
			systemLastSeenRx, systemLastSeenTx int64
			systemBootTimeUnix                 int64
		}
		settings := make(map[int64]nodeSet)
		if srows, serr := r.db.QueryContext(ctx, `SELECT id, COALESCE(lock_entry_ip,0), COALESCE(port_range_min,0), COALESCE(port_range_max,0), COALESCE(include_in_traffic_stats,1), COALESCE(system_rx_cycle,0), COALESCE(system_tx_cycle,0), COALESCE(system_last_seen_rx,0), COALESCE(system_last_seen_tx,0), COALESCE(system_boot_time_unix,0) FROM remote_servers`); serr == nil {
			for srows.Next() {
				var id int64
				var lockInt, includeStatsInt int
				var ns nodeSet
				if srows.Scan(&id, &lockInt, &ns.min, &ns.max, &includeStatsInt, &ns.systemRxCycle, &ns.systemTxCycle, &ns.systemLastSeenRx, &ns.systemLastSeenTx, &ns.systemBootTimeUnix) == nil {
					ns.lock = lockInt != 0
		...
		for i := range servers {
			if ns, ok := settings[servers[i].ID]; ok {
				...
				servers[i].SystemRxCycle = ns.systemRxCycle
				servers[i].SystemTxCycle = ns.systemTxCycle
				servers[i].SystemLastSeenRx = ns.systemLastSeenRx
				servers[i].SystemLastSeenTx = ns.systemLastSeenTx
				servers[i].SystemBootTimeUnix = ns.systemBootTimeUnix
			}
		}
		// 探针元数据独立补查，避免继续扩张上方易错的 positional Scan。
		if mrows, merr := r.db.QueryContext(ctx, `SELECT id, COALESCE(region,''), COALESCE(region_country,''), COALESCE(region_name,''), COALESCE(region_city,''), COALESCE(renewal_cycle,'month'), expires_at, COALESCE(provider_name,''), COALESCE(provider_url,''), COALESCE(telecom_paid_peer,0), provider_updated_at FROM remote_servers`); merr == nil {
			...
			for mrows.Next() {
				...
				var region, regionCountry, regionName, regionCity, cycle string
				var providerName, providerURL string
				var telecomPaidPeer bool
				if mrows.Scan(&id, &region, &regionCountry, &regionName, &regionCity, &cycle, &expires, &providerName, &providerURL, &telecomPaidPeer, &providerUpdated) == nil {
					if s := byID[id]; s != nil {
						s.Region, s.RenewalCycle = region, cycle
						s.RegionCountry, s.RegionName, s.RegionCity = regionCountry, regionName, regionCity
...
func (r *TrafficRepository) GetRemoteServer(ctx context.Context, id int64) (*RemoteServer, error) {
	...
	var ddnsLastSyncedAt sql.NullTime
	var lastTrafficResetAt sql.NullTime
	err := r.db.QueryRowContext(ctx, query, id).Scan(&server.ID, &server.Name, &server.Token, &server.Status, &lastHeartbeat, &server.IPAddress, &server.IPAddressV6, &server.IPv6Enabled, &server.OfflineNotified, &server.Domain, &server.DomainV6,
		&bootTime, &xrayBootTime, &server.BootCount, &server.XrayBootCount,
		&tokenExpiresAt, &lastTokenRefresh,
		&server.ConnectionMode, &server.PullAddress, &server.PullAddressV6, &server.PullPort, &server.PullToken, &lastPullAt,
		&server.ListenPort,
	...
	var expires, providerUpdated sql.NullTime
	_ = r.db.QueryRowContext(ctx, `SELECT COALESCE(region,''), COALESCE(region_country,''), COALESCE(region_name,''), COALESCE(region_city,''), COALESCE(renewal_cycle,'month'), expires_at, COALESCE(provider_name,''), COALESCE(provider_url,''), COALESCE(telecom_paid_peer,0), provider_updated_at FROM remote_servers WHERE id = ?`, id).
		Scan(&server.Region, &server.RegionCountry, &server.RegionName, &server.RegionCity, &server.RenewalCycle, &expires, &server.ProviderName, &server.ProviderURL, &server.TelecomPaidPeer, &providerUpdated)
	if expires.Valid {
		t := expires.Time
		server.ExpiresAt = &t
	}
...
// UpdateRemoteServerProbeMeta 更新服务管理与探针共用的地域和到期信息。
func (r *TrafficRepository) UpdateRemoteServerProbeMeta(ctx context.Context, id int64, region, cycle string, expiresAt *time.Time) error {
	if id <= 0 {
	...
	result, err := r.db.ExecContext(ctx, `UPDATE remote_servers SET region=?, renewal_cycle=?, expires_at=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
		strings.TrimSpace(region), cycle, expiresAt, id)
	if err != nil {
...
func (r *TrafficRepository) ConfirmRemoteServerRenewal(ctx context.Context, id int64, expectedDate string) (*RemoteServer, bool, error) {
	...
	// 始终从原到期日顺延，保留用户配置的账单日。若主控离线跨过了多个
	// 周期，则一次补到未来最近的周期，避免以“确认当天”为基准造成日期漂移。
	newExpiry := base.AddDate(0, months, 0)
	for !newExpiry.After(now) {
		newExpiry = newExpiry.AddDate(0, months, 0)
	}
	result, err := r.db.ExecContext(ctx,
...
func (r *TrafficRepository) UpdateRemoteServerLocation(ctx context.Context, id int64, country, region, city string) error {
	...
	result, err := r.db.ExecContext(ctx, `UPDATE remote_servers SET region_country=?, region_name=?, region_city=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
		strings.ToUpper(strings.TrimSpace(country)), strings.TrimSpace(region), strings.TrimSpace(city), id)
	if err != nil {
		return fmt.Errorf("update server location: %w", err)
	}
	if n, _ := result.RowsAffected(); n == 0 {
		return ErrRemoteServerNotFound
	}
...
func (r *TrafficRepository) GetRemoteServerByToken(ctx context.Context, token string) (*RemoteServer, error) {
	...
	var ddnsEnabledInt, ddnsPendingInt int
	var ddnsLastSyncedAt sql.NullTime
	err := r.db.QueryRowContext(ctx, query, token).Scan(&server.ID, &server.Name, &server.Token, &server.Status, &lastHeartbeat, &server.IPAddress, &server.IPAddressV6, &server.IPv6Enabled, &server.OfflineNotified, &server.Domain, &server.DomainV6,
		&bootTime, &xrayBootTime, &server.BootCount, &server.XrayBootCount,
		&tokenExpiresAt, &lastTokenRefresh,
		&server.ConnectionMode, &server.PullAddress, &server.PullAddressV6, &server.PullPort, &server.PullToken, &lastPullAt,
		&server.AgentToken, &agentTokenExpiresAt, &lastAgentTokenRefresh,
...
func (r *TrafficRepository) GetRemoteServerByName(ctx context.Context, name string) (*RemoteServer, error) {
	...
	var ddnsEnabledInt, ddnsPendingInt int
	var ddnsLastSyncedAt sql.NullTime
	err := r.db.QueryRowContext(ctx, query, name).Scan(&server.ID, &server.Name, &server.Token, &server.Status, &lastHeartbeat, &server.IPAddress, &server.IPAddressV6, &server.IPv6Enabled, &server.OfflineNotified, &server.Domain, &server.DomainV6,
		&bootTime, &xrayBootTime, &server.BootCount, &server.XrayBootCount,
		&tokenExpiresAt, &lastTokenRefresh,
		&server.ConnectionMode, &server.PullAddress, &server.PullAddressV6, &server.PullPort, &server.PullToken, &lastPullAt,
		&server.ListenPort,
...
func (r *TrafficRepository) CreateRemoteServer(ctx context.Context, server *RemoteServer) error {
	...
	result, err := r.db.ExecContext(ctx, stmt, server.Name, server.Token, server.Status, server.IPAddress, server.IPAddressV6, server.IPv6Enabled, server.Domain, server.DomainV6, tokenExpiresAt, server.ConnectionMode, server.ListenPort, server.PullAddress, server.PullAddressV6, server.PullPort, server.PullToken, server.Use443, stealMode, server.SiteType, server.SiteValue, xrayMode, server.TrafficLimit, server.TrafficUsedOffset, server.TrafficResetDay, statsMode, trafficSource, ddnsEnabledInt, server.DDNSProviderID)
	if err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "unique") {
...
func (r *TrafficRepository) UpdateRemoteServerHeartbeatWithRestart(ctx context.Context, update HeartbeatUpdate) (*HeartbeatResult, error) {
	...
	// boot_time 归一到 UTC 后再比较/写入。
	//
	// postgres 的 boot_time 是 `timestamp`(不带时区)列:写入一个带 +08:00 的 time.Time
	// 会被剥掉时区、只存墙钟 "17:37:32";读回是无时区字符串,parseNullTimeString 按 UTC 解析
	// → 与 agent 原始 +08:00 值的 .Unix() 差了整整一个时区偏移(如 8 小时)。
	// 于是 update.BootTime.Unix() != server.BootTime.Unix() 恒为真,每次心跳都误判 mmwx 重启,
	// boot_count 无限递增、日志刷屏(许可证站 #262)。SQLite 无此问题,但统一 UTC 对两方言都正确。
	if update.BootTime != nil {
		u := update.BootTime.UTC()
		update.BootTime = &u
	}
	if update.XrayBootTime != nil {
		u := update.XrayBootTime.UTC()
		update.XrayBootTime = &u
	}

	// 检测mmwx重启
	if update.BootTime != nil {
		if server.BootTime != nil && update.BootTime.Unix() != server.BootTime.Unix() {
	...
	// 检测 X 射线重启
	if update.XrayBootTime != nil {
		if server.XrayBootTime != nil && update.XrayBootTime.Unix() != server.XrayBootTime.Unix() {
...
// RestoreServerToken 把 server token 写回指定值，用于令牌轮换失败时的补偿回滚。
//
// 与 ResetServerToken 的区别：不生成新令牌，也**不**推进 last_token_refresh ——
// 这次轮换从未真正生效，刷新时间戳不该被推进，否则会顶掉 Agent 自助刷新的 24 小时窗口。
func (r *TrafficRepository) RestoreServerToken(ctx context.Context, serverID int64, token string, expiresAt *time.Time) error {
	if r == nil || r.db == nil {
		return errors.New("traffic repository not initialized")
	}
	...
	result, err := r.db.ExecContext(ctx, stmt, token, expiresAt, serverID)
	if err != nil {
		return fmt.Errorf("restore server token: %w", err)
	}
...
func (r *TrafficRepository) UpdateRemoteServer(ctx context.Context, id int64, name, domain, domainV6 string, trafficLimit int64, trafficResetDay int, connectionMode, xrayMode, trafficStatsMode, trafficSource string, ipv6Enabled *bool) error {
	if r == nil || r.db == nil {
		return errors.New("traffic repository not initialized")
	}
	...
	// 动态构建 SET 子句
	setClauses := []string{"name = ?", "domain = ?", "domain_v6 = ?", "traffic_limit = ?", "traffic_reset_day = ?"}
	args := []any{name, domain, domainV6, trafficLimit, trafficResetDay}
...
func (r *TrafficRepository) resetUserTrafficCycle(ctx context.Context, username string, resetAt *time.Time) error {
	...
	if err := refreshPackageNodeTrafficBaselinesTx(ctx, tx, username); err != nil {
		return fmt.Errorf("reset package node traffic limits: %w", err)
	}
	if resetAt != nil {
...
func (r *TrafficRepository) UpdateUserResetDay(ctx context.Context, username string, day int) error {
	...
	_, _ = r.db.ExecContext(ctx, `UPDATE user_package_assignments SET reset_day=?,updated_at=CURRENT_TIMESTAMP WHERE username=? AND is_primary=1 AND status='active'`, day, username)
	return nil
}
