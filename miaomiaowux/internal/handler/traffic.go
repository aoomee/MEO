...
func (h *TrafficHandler) handleUserConnections(w http.ResponseWriter, r *http.Request) {
	...
	h.writeJSON(w, http.StatusOK, map[string]any{
		"success": true, "connections": AggregateUserConnCounts(),
		"connection_count_ready": ready, "not_ready_server_ids": notReady,
	})
}
...
type RemoteTrafficRequest struct {
	...
	// 连接统计状态与 WS traffic payload 对齐。指针用于区分旧 Agent 未上报和明确未就绪。
...
func (h *RemoteTrafficHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// HTTP/pull 模式与 WS 模式共享同一份内存快照，避免有实际连接时 API 仍显示 0。
	connCountsByServer.Store(serverID, req.ConnCounts)
	nodeConnCountsByServer.Store(serverID, req.NodeConnCounts)
	if req.ConnectionCountReady != nil {
		connCountReadyByServer.Store(serverID, *req.ConnectionCountReady)
	}
