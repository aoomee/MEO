...
func NewUserTokenHandler(repo *storage.TrafficRepository, remoteManage *RemoteManageHandler) http.Handler {
	if repo == nil {
		panic("user token handler requires repository")
	}

	return &userTokenHandler{repo: repo, remoteManage: remoteManage}
}
...
func (h *userTokenHandler) handleReset(w http.ResponseWriter, r *http.Request, username string) {
	...
	reset, err := resetUserSubscriptionCredentials(r.Context(), h.repo, h.remoteManage, user)
	if err != nil {
		writeError(w, http.StatusBadGateway, err)
		return
	}

	effective, _ := h.repo.GetEffectiveUserShortCode(r.Context(), username)
	customCode, _ := h.repo.GetUserCustomShortCode(r.Context(), username)
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(map[string]any{
		"token":                  reset.Token,
		"user_short_code":        effective,
		"custom_user_short_code": customCode,
