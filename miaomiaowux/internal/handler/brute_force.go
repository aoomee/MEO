...
type BruteForceProtector struct {
	...
	skipLocalIP bool
	// ipWhitelist is shared with the subscription rate limiter. It contains
	// administrator configured IP/CIDR entries plus managed Agent public IPs.
...
func (p *BruteForceProtector) SetIPWhitelist(contains func(string) bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.ipWhitelist = contains
}

func (p *BruteForceProtector) IsWhitelisted(ip string) bool {
	p.mu.RLock()
	contains := p.ipWhitelist
	p.mu.RUnlock()
	return contains != nil && contains(ip)
}
...
func (p *BruteForceProtector) shouldSkip(ip string) bool {
	...
	contains := p.ipWhitelist
	p.mu.RUnlock()
	return (contains != nil && contains(ip)) || (skip && IsLocalOrPrivateIP(ip))
}
...
func (p *BruteForceProtector) RestoreFromDB(ctx context.Context) {
	...
	n := 0
	for _, b := range bans {
		if p.IsWhitelisted(b.IP) {
			_ = repo.ReleaseIPBan(ctx, b.IP, "system-whitelist")
			_ = repo.InsertSecurityEvent(ctx, storage.SecurityEvent{
				IP: b.IP, Kind: "whitelist_unban", Actor: "system-whitelist",
			})
			continue
		}
...
// ReconcileWhitelist immediately removes in-memory and persisted bans that are
// now covered by the whitelist. This is called after an administrator saves the
// list and whenever a managed Agent reports a different public IP.
func (p *BruteForceProtector) ReconcileWhitelist(ctx context.Context, actor string) int {
	if p == nil {
		return 0
	}
	p.attempts.Range(func(key, _ any) bool {
		ip, _ := key.(string)
		if p.IsWhitelisted(ip) {
	...
	bans, err := repo.ListActiveIPBans(ctx)
	if err != nil {
		return 0
	}
	released := 0
	for _, ban := range bans {
		if !p.IsWhitelisted(ban.IP) {
			continue
		}
		if err := repo.ReleaseIPBan(ctx, ban.IP, actor); err != nil {
			continue
		}
		_ = repo.InsertSecurityEvent(ctx, storage.SecurityEvent{
			IP: ban.IP, Kind: "whitelist_unban", Actor: actor,
		})
		released++
	}
	return released
