...
func (h *TrafficSummaryHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if isAdmin {
		...
		// 普通用户:汇总其所有套餐实例。每个实例的凭据和流量基线独立,
		// 因此同一节点出现在两个套餐时也不会把同一份流量重复归属。
		assignments, assignmentErr := h.repo.ListUserPackageAssignments(ctx, username, false)
		if assignmentErr == nil && len(assignments) > 0 {
			for _, assignment := range assignments {
				totalLimit += assignment.EffectiveTrafficLimit()
				if billable, terr := h.repo.GetPackageAssignmentBillableTraffic(ctx, assignment.ID); terr == nil {
					totalUsed += billable
				}
			}
		} else if user.PackageID > 0 { // 迁移失败时保留旧数据口径,不让首页瞬间归零。
