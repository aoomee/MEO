...
func getGeoIPCountryCode(ipOrHost string) string {
	...
	// GeoIP 是可选增强功能。未由管理员显式配置时不向任何第三方
	// 发送节点 IP，避免隐式遥测和共享账号凭据。
	ipInfoToken := strings.TrimSpace(os.Getenv("MEO_IPINFO_TOKEN"))
	if ipInfoToken == "" {
		geoIPCache.Store(ip, "")
		return ""
	}
	...
	endpoint := "https://api.ipinfo.io/lite/" + url.PathEscape(ip) + "?token=" + url.QueryEscape(ipInfoToken)
