...
func (h *SubscriptionHandler) generateFromTemplate(ctx context.Context, subscribeFile storage.SubscribeFile) ([]byte, error) {
	...
	// 节点池选择:
	//   - 订阅创建者是普通用户且绑了套餐 → 本人导入节点 + 套餐可见节点
	//   - 订阅创建者是管理员 / 普通用户没套餐 / 没找到用户 → 沿用旧行为(所有节点)
	// 普通用户的节点池必须与节点管理页一致。此前这里只加载 pkg.Nodes，导致用户在
	// 编辑订阅时明明选中了自己导入的节点，生成订阅时却在 ID 过滤前就把它丢掉。
	var nodes []storage.Node
	creator := strings.TrimSpace(subscribeFile.CreatedBy)
	restrictToPackage := false
	packageVisibleNodeIDs := map[int64]bool{}
	if creator != "" {
		if user, uerr := h.repo.GetUser(ctx, creator); uerr == nil && user.Role != storage.RoleAdmin && user.PackageID > 0 {
			if pkg, perr := h.repo.GetPackage(ctx, user.PackageID); perr == nil && pkg != nil {
				...
				effectiveNodes, effectiveErr := effectivePackageNodeIDs(ctx, h.repo, pkg.Nodes, pkg.NodesConfigured)
				if effectiveErr != nil {
					return nil, fmt.Errorf("获取套餐节点列表失败: %w", effectiveErr)
				}
				for _, nid := range effectiveNodes {
					packageVisibleNodeIDs[nid] = true
				}
				var nerr error
				nodes, nerr = collectUserVisibleNodes(ctx, h.repo, creator)
				if nerr != nil {
					return nil, fmt.Errorf("获取用户可见节点列表失败: %w", nerr)
				}
				// 套餐父节点派生的 shared routed 子节点同样属于套餐范围。
				for _, node := range nodes {
					if node.ParentNodeID != nil && node.RoutedOwner == "shared" && packageVisibleNodeIDs[*node.ParentNodeID] {
						packageVisibleNodeIDs[node.ID] = true
					}
				}
			}
		}
	...
	// 套餐节点仍以当前 pkg.Nodes 为真源，避免套餐后续新增节点被旧快照过滤；用户私有
	// 导入节点则必须显式出现在 selected_node_ids 中才进入订阅。这样既保留套餐自动扩容
	// 行为，也让“编辑订阅 → 选择本人导入节点”真正生效。
	...
	buildProxy := func(node storage.Node) (map[string]any, bool) {
		...
		if restrictToPackage && node.NodeType == "routed" {
			...
			// 只有套餐节点需要换成本用户的入站凭据；本人导入的外部节点必须保留
			// 导入时携带的原始凭据。
			if credMap != nil && packageVisibleNodeIDs[node.ID] {
				applyUserCredentials(proxyConfig, node, credMap)
			}
			applyWGLeaseToProxy(ctx, h.repo, proxyConfig, node, creator)
		}
	...
	includeNode := func(node storage.Node) bool {
		if restrictToPackage && !packageVisibleNodeIDs[node.ID] && !selectedNodeIDsMap[node.ID] {
			return false
		}
...
func GetExternalSubscriptionsFromFile(ctx context.Context, data []byte, username string, repo *storage.TrafficRepository) (map[string]bool, error) {
	...
	if proxies, ok := yamlContent["proxies"].([]any); ok {
		...
		if len(proxyNames) > 0 {
			...
			// MEO模式：通过节点的 Tag（外部订阅名称）找到外部订阅URL
			if len(usedTags) > 0 {
	...
	if proxyGroups, ok := yamlContent["proxy-groups"].([]any); ok {
		...
		groupNames := make(map[string]bool) // MEO模式：收集 proxy-group 的名称
		for _, group := range proxyGroups {
			if groupMap, ok := group.(map[string]any); ok {
				// 收集 proxy-group 名称（MEO模式会创建同名的 proxy-group）
...
// ─── 订阅 YAML → JSON 序列化(从MEO subscription.go L2506-2679 移植,无 mmw 特化) ───
//
