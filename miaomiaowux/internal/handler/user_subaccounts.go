...
func (h *UserSubaccountsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if routed, err := h.repo.ListPackageAssignmentSubaccountsByUser(ctx, username); err == nil {
		for _, sa := range routed {
			node := nodeByID[sa.RoutedNodeID]
			it := subaccountItem{AssignmentID: sa.AssignmentID, Type: "routed", Email: sa.Email, Identifier: identifierFromCredJSON(sa.CredentialJSON), NodeID: sa.RoutedNodeID, NodeName: node.NodeName, IsActive: sa.IsActive, UpdatedAt: sa.UpdatedAt.Format("2006-01-02 15:04:05"), InboundTag: node.InboundTag}
			if node.OriginalServer != "" {
				it.ServerName = node.OriginalServer
				it.ServerID = serverByName[node.OriginalServer]
			}
	...
	if confs, err := h.repo.ListPackageAssignmentInboundConfigsByUser(ctx, username); err == nil {
		for _, c := range confs {
			items = append(items, subaccountItem{AssignmentID: c.AssignmentID, Type: "inbound", Email: c.Email, Identifier: identifierFromCredJSON(c.CredentialJSON), ServerID: c.ServerID, ServerName: serverByID[c.ServerID], InboundTag: c.InboundTag, Protocol: c.Protocol, IsActive: true, UpdatedAt: c.CreatedAt.Format("2006-01-02 15:04:05")})
		}
	}
