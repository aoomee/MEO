...
func (h *shortLinkHandler) TryServe(w http.ResponseWriter, r *http.Request) bool {
	...
	// Multi-package links identify the assigned package directly. The
	// random assignment short code is both unambiguous (even when the same
	// package is bought twice) and independent from the user's login token.
	if assignment, err := h.repo.GetUserPackageAssignmentByShortCode(ctx, code); err == nil && assignment != nil {
		if h.packageHandler == nil {
			return false
		}
		newURL := *r.URL
		q := newURL.Query()
		q.Set("assignment", assignment.ShortCode)
		newURL.RawQuery = q.Encode()
		newCtx := auth.ContextWithUsername(ctx, assignment.Username)
	...
	for i := len(code) - 1; i >= 1; i-- {
		...
		if packageID, pOk := packageCodes[leftCode]; pOk {
			username = un
	...
	if isPackage && h.packageHandler != nil {
		assignments, _ := h.repo.ListUserPackageAssignments(ctx, username, false)
		var assignmentID int64
		for _, assignment := range assignments {
			if assignment.PackageID == matchedPackageID {
		...
		newURL := *r.URL
		q := newURL.Query()
		q.Set("assignment_id", fmt.Sprint(assignmentID))
		newURL.RawQuery = q.Encode()
		newCtx := auth.ContextWithUsername(ctx, username)
		newRequest := r.Clone(newCtx)
		newRequest.URL = &newURL
