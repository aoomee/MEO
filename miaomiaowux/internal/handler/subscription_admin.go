...
func NewSubscriptionListHandler(repo *storage.TrafficRepository) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if user.Role == storage.RoleAdmin {
			...
			// 套餐订阅由套餐实例动态生成。旧版自动写入 subscribe_files 的
			// package 行会与实例卡片重复，普通用户列表中不再返回。
			manual := files[:0]
			for _, file := range files {
				if file.Type != storage.SubscribeTypePackage {
					manual = append(manual, file)
				}
			}
			files = manual
			if assignments, aerr := repo.ListUserPackageAssignments(r.Context(), username, false); aerr == nil {
				for _, assignment := range assignments {
					files = append(files, storage.SubscribeFile{
						ID: -assignment.ID, Name: assignment.PackageName,
						Description: assignment.PackageDescription, Type: storage.SubscribeTypePackage,
						Filename: fmt.Sprintf("__package__:%d", assignment.ID), FileShortCode: assignment.ShortCode,
						ExpireAt: assignment.PackageEndDate, UpdatedAt: assignment.UpdatedAt,
					})
				}
			}
			...
			if own, oerr := repo.ListSubscribeFiles(r.Context()); oerr == nil {
				for _, f := range own {
					if f.CreatedBy == username && f.Type != storage.SubscribeTypePackage && !seen[f.ID] {
