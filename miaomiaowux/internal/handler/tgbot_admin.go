...
func (h *TGBotAPIHandler) userSummary(w http.ResponseWriter, r *http.Request) {
	...
	assignments, _ := h.repo.ListUserPackageAssignments(r.Context(), username, false)
	packageItems := make([]map[string]any, 0, len(assignments))
	for _, assignment := range assignments {
		up, down, _ := h.repo.GetPackageAssignmentBillableTrafficByDirection(r.Context(), assignment.ID)
		item := map[string]any{
			"assignment_id":    assignment.ID,
			"name":             assignment.PackageName,
			"traffic_limit":    assignment.EffectiveTrafficLimit(),
			"traffic_limit_gb": float64(assignment.EffectiveTrafficLimit()) / (1024 * 1024 * 1024),
			"cycle_uplink":     up,
			"cycle_downlink":   down,
			"is_primary":       assignment.IsPrimary,
		}
		if assignment.PackageEndDate != nil {
			item["end_date"] = assignment.PackageEndDate.Format(time.RFC3339)
		}
		packageItems = append(packageItems, item)
	}
	out["packages"] = packageItems
	// 套餐
	if user.PackageID > 0 {
...
func (h *TGBotAPIHandler) userSubscriptions(w http.ResponseWriter, r *http.Request) {
	...
	defaultSubscriptions := make([]subOut, 0)
	if assignments, aerr := h.repo.ListUserPackageAssignments(r.Context(), username, false); aerr == nil {
		for _, assignment := range assignments {
			defaultSubscriptions = append(defaultSubscriptions, subOut{
				Name: assignment.PackageName, Description: "套餐默认订阅", CombinedCode: assignment.ShortCode,
			})
		}
	}
	resp["default_subscriptions"] = defaultSubscriptions
...
func (h *TGBotAPIHandler) userNodes(w http.ResponseWriter, r *http.Request) {
	...
	effectiveNodes, _ := effectivePackageNodeIDs(r.Context(), h.repo, pkg.Nodes, pkg.NodesConfigured)
	out := make([]nodeOut, 0, len(effectiveNodes))
...
func (h *TGBotAPIHandler) notifyDigest(w http.ResponseWriter, r *http.Request) {
	...
	type digestUser struct {
		...
		TodayUplink    int64   `json:"today_uplink"`
		TodayDownlink  int64   `json:"today_downlink"`
	...
	for _, t := range targets {
		...
		if daily, dailyErr := h.repo.ListUserDailyBillableTraffic(ctx, t.Username, today, today); dailyErr == nil && len(daily) > 0 {
			du.TodayUplink = int64(daily[0].Uplink)
			du.TodayDownlink = int64(daily[0].Downlink)
		}
...
// 数据直接来自每日计费账本，与主控 /api/admin/traffic/period?view=users 相同。
func (h *TGBotAPIHandler) userDailyTraffic(w http.ResponseWriter, r *http.Request) {
	...
	if user, err := h.repo.GetUser(ctx, username); err == nil && user.PackageID > 0 {
		if pkg, perr := h.repo.GetPackage(ctx, user.PackageID); perr == nil && pkg != nil && pkg.CycleDays > 0 {
			startDay = today.AddDate(0, 0, -pkg.CycleDays)
		}
	}
	if periodStart, _, periodErr := h.repo.GetUserPackagePeriod(ctx, username); periodErr == nil && periodStart != nil {
		localStart := periodStart.In(now.Location())
		startDay = time.Date(localStart.Year(), localStart.Month(), localStart.Day(), 0, 0, 0, 0, now.Location())
	}
	// Keep the compact TG chart bounded while always retaining today.
	oldest := today.AddDate(0, 0, -61)
	if startDay.Before(oldest) {
		startDay = oldest
	}
	if startDay.After(today) {
		startDay = today
	}
	records, err := h.repo.ListUserDailyBillableTraffic(ctx, username, startDay.Format("2006-01-02"), today.Format("2006-01-02"))
	if err != nil {
		writeJSONError(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	byDate := make(map[string]float64, len(records))
	for _, rec := range records {
		byDate[rec.Date] = (rec.Uplink + rec.Downlink) / (1024 * 1024 * 1024)
	}
	out := make([]dayUsage, 0, int(today.Sub(startDay).Hours()/24)+1)
	for day := startDay; !day.After(today); day = day.AddDate(0, 0, 1) {
		date := day.Format("2006-01-02")
		out = append(out, dayUsage{Date: date, UsedGB: byDate[date]})
	}
