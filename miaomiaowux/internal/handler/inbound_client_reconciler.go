...
func findMissingInboundClients(
	...
	return findMissingInboundClientsForAssignments(configs, snapshots, eligible, nil)
}

func findMissingInboundClientsForAssignments(
	configs []storage.UserInboundConfig,
	snapshots map[int64]map[string]map[string]bool,
	eligibleUsers map[string]bool,
	eligibleAssignments map[int64]bool,
) []reconcileTarget {
	var out []reconcileTarget
	for _, cfg := range configs {
		if cfg.AssignmentID > 0 {
			if !eligibleAssignments[cfg.AssignmentID] {
				continue
			}
		} else if !eligibleUsers[cfg.Username] {
			continue
		}
...
func (c *InboundClientReconciler) buildEligibleAssignments(ctx context.Context) map[int64]bool {
	assignments, err := c.repo.ListActivePackageAssignments(ctx)
	if err != nil {
		return nil
	}
	now := time.Now()
	out := make(map[int64]bool, len(assignments))
	for _, assignment := range assignments {
		if assignment.Legacy || assignment.OverLimitEnforced || (assignment.PackageEndDate != nil && now.After(*assignment.PackageEndDate)) {
			continue
		}
		user, err := c.repo.GetUser(ctx, assignment.Username)
		if err == nil && user.IsActive {
			out[assignment.ID] = true
		}
	}
	return out
}
...
func (c *InboundClientReconciler) runOnce(ctx context.Context) (string, error) {
	...
	missing := findMissingInboundClientsForAssignments(configs, snapshots, eligible, c.buildEligibleAssignments(ctx))
	if len(missing) == 0 {
