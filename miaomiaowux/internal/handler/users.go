...
func NewUserStatusHandler(repo *storage.TrafficRepository, remoteManage *RemoteManageHandler, pusher *LimiterConfigPusher, tokens *auth.TokenStore) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if payload.IsActive {
			packageValid := false
			if assignments, listErr := repo.ListUserPackageAssignments(ctx, username, false); listErr == nil {
				for _, assignment := range assignments {
					if !assignment.OverLimitEnforced && (assignment.PackageEndDate == nil || assignment.PackageEndDate.After(time.Now())) {
						packageValid = true
						break
					}
				}
			}
			shouldSyncAccess = packageValid
		}
...
func NewUserLimitsHandler(repo *storage.TrafficRepository, pusher *LimiterConfigPusher, licenseManager *license.Manager) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// 设置非空 SpeedLimitOverride 时检查本地限速能力。
