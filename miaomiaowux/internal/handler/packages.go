...
func validatePackageNodeTrafficLimits(limits map[int64]float64, nodes []int64, packageLimitGB float64) error {
	allowed := make(map[int64]bool, len(nodes))
	for _, id := range nodes {
		allowed[id] = true
	}
	for id, limit := range limits {
		if !allowed[id] {
			// 节点已被删除但套餐里的 per-node 额度残留(0.4.8-beta.19 之前删节点不清理
			// packages.node_traffic_limits)。这里原本直接报错,后果是该套餐**永远保存不了**:
			// 前端把套餐读出来原样回传,里面必然带着这个已删节点的额度。
			// 与上面 validatePackageNodeNames 保持一致 —— 跳过即可,额度本身已无意义。
			continue
		}
		if limit < 0 || math.IsNaN(limit) || math.IsInf(limit, 0) {
			return fmt.Errorf("节点 %d 的流量额度无效", id)
		}
		// 0 表示不启用单节点额度；启用时它是套餐总额度之下的第二道限制，
		// 超过总额度永远不可达，只会制造误导性配置。
		if limit > 0 && limit > packageLimitGB {
			return fmt.Errorf("节点 %d 的流量额度 %.2f GB 不能大于套餐流量额度 %.2f GB", id, limit, packageLimitGB)
		}
	}
	return nil
}
...
func (h *PackageCreateHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if (req.SpeedLimitMbps > 0 || len(req.AutoSpeedRules) > 0 || hasNonZeroLimit(req.NodeSpeedLimits) || hasNonZeroIntLimit(req.NodeDeviceLimits)) && h.licenseManager != nil && !h.licenseManager.HasFeature("limiter") {
		http.Error(w, "本地限速功能不可用", http.StatusForbidden)
		return
	}
	...
	if err := validatePackageNodeTrafficLimits(req.NodeTrafficLimits, nodes, req.TrafficLimitGB); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (h *PackageUpdateHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// nodes 是管理员本次提交的选择:提交空列表就是明确选择"放行全部节点",
	// 因此这里传 false(不是"配置过但被删空")。
	newSyncNodes, effectiveErr := effectivePackageNodeIDs(r.Context(), h.repo, nodes, false)
	if effectiveErr != nil {
		http.Error(w, effectiveErr.Error(), http.StatusInternalServerError)
		return
	}
	oldNodesConfigured := oldPkg != nil && oldPkg.NodesConfigured
	oldSyncNodes, effectiveErr := effectivePackageNodeIDs(r.Context(), h.repo, oldNodes, oldNodesConfigured)
	if effectiveErr != nil {
		http.Error(w, effectiveErr.Error(), http.StatusInternalServerError)
		return
	}
	if len(oldNodes) == 0 && len(nodes) == 0 {
		// Empty previously meant “all” in the UI but provisioned nothing. Force an
		// idempotent reconciliation so existing affected packages repair themselves.
		oldSyncNodes = nil
	}
	...
	defer syncCancel()
	syncErr := h.syncPackageNodesTransactionally(syncCtx, req.ID, oldSyncNodes, newSyncNodes)
	if syncErr != nil {
		...
		for _, nodeID := range removedNodes {
			...
			go func(user storage.User, nodeID int64) {
				...
				if node.NodeType == "routed" {
					...
					if err := removeProvisionedUserFromRoutedNode(ctx, h.remoteManage, h.repo, user, node.ID); err != nil {
...
func (h *PackageDeleteHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	assignments, err := h.repo.ListActivePackageAssignmentsByPackage(ctx, id)
	if err != nil {
	...
	for _, assignment := range assignments {
		base, userErr := h.repo.GetUser(ctx, assignment.Username)
		if errors.Is(userErr, storage.ErrUserNotFound) {
			// 同上:孤儿关联跳过,否则整个套餐永远删不掉。
			log.Printf("[套餐删除] 跳过孤儿套餐关联: 用户 %s 已不存在", assignment.Username)
			continue
		}
		if userErr != nil {
			http.Error(w, "读取套餐用户失败: "+userErr.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, assignment := range assignments {
		...
		if err := h.repo.RemovePackageFromUser(ctx, assignment.Username); err != nil {
			databaseErr = fmt.Errorf("清除用户 %s 套餐绑定: %w", assignment.Username, err)
			break
		}
	}
	if databaseErr == nil {
	...
	pushedUsers := map[string]bool{}
	for _, user := range targetUsers {
		if h.pusher != nil {
			if !pushedUsers[user.Username] {
				pushedUsers[user.Username] = true
				go h.pusher.PushToAllServersForUser(context.Background(), user.Username)
			}
		}
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"unbound_users": len(pushedUsers),
		"assignments":   len(targetUsers),
	})
}
...
type assignPackageRequest struct {
	...
	IsReset  *bool `json:"is_reset"`
	ResetDay *int  `json:"reset_day"`
	// 两项默认 false。仅切换到不同套餐时生效；同套餐续期本来就会保留流量。
	InheritExpireDate bool `json:"inherit_expire_date"`
	InheritTraffic    bool `json:"inherit_traffic"`
}

type PackageAssignOptions struct {
	InheritExpireDate bool
	InheritTraffic    bool
}
...
func (h *PackageAssignHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	warnings, perr := h.AssignAndProvisionWithOptions(ctx, req.Username, req.PackageID, startDate, endDate, isReset, resetDay, PackageAssignOptions{
		InheritExpireDate: req.InheritExpireDate,
		InheritTraffic:    req.InheritTraffic,
	})
	if perr != nil {
		...
		if errors.Is(perr, storage.ErrPackageAlreadyAssignedIndependently) {
			// 不是服务端故障,是管理员操作冲突 —— 用 400 + 明确文案,避免前端只显示一句
			// 无从下手的「服务器错误」。
			http.Error(w, "该套餐已作为独立套餐绑定给此用户，无法再设为主套餐；如需调整请先解绑对应的独立套餐", http.StatusBadRequest)
			return
		}
...
func (h *PackageAssignHandler) AssignAndProvision(ctx context.Context, username string, packageID int64, startDate, endDate time.Time, isReset bool, resetDay int) ([]string, error) {
	return h.AssignAndProvisionWithOptions(ctx, username, packageID, startDate, endDate, isReset, resetDay, PackageAssignOptions{})
}

func (h *PackageAssignHandler) AssignAndProvisionWithOptions(ctx context.Context, username string, packageID int64, startDate, endDate time.Time, isReset bool, resetDay int, options PackageAssignOptions) ([]string, error) {
	if primary, primaryErr := h.repo.GetPrimaryUserPackageAssignment(ctx, username); primaryErr == nil && primary != nil && !primary.Legacy {
		return h.updateIndependentPrimary(ctx, *primary, packageID, startDate, endDate, isReset, resetDay, options)
	}
	...
	var oldNodes []int64
	packageChanged := snapshot.PackageID == nil || *snapshot.PackageID != packageID
	...
	if packageChanged {
		if options.InheritExpireDate && snapshot.PackageEndDate != nil {
			endDate = *snapshot.PackageEndDate
		}
		// 继承流量时也保留原周期起点，让每日账单图覆盖被继承的用量；
		// last_reset_at 会在存储事务中标记为本次切换时间，避免新重置日已过时立即清零。
		if options.InheritTraffic && snapshot.PackageStartDate != nil {
			startDate = *snapshot.PackageStartDate
		}
	}
	...
	if err := h.repo.AssignPackageToUserWithOptions(ctx, username, packageID, startDate, endDate, isReset, resetDay, storage.PackageAssignmentOptions{
		InheritTraffic: packageChanged && options.InheritTraffic,
	}); err != nil {
		return nil, err
	}
...
func (h *PackageAssignHandler) updateIndependentPrimary(ctx context.Context, current storage.UserPackageAssignment, packageID int64, startDate, endDate time.Time, isReset bool, resetDay int, options PackageAssignOptions) ([]string, error) {
	base, err := h.repo.GetUser(ctx, current.Username)
	if err != nil {
		return nil, err
	}
	oldPackage, err := h.repo.GetPackage(ctx, current.PackageID)
	if err != nil {
		return nil, err
	}
	newPackage, err := h.repo.GetPackage(ctx, packageID)
	...
	packageChanged := current.PackageID != packageID
	if packageChanged && options.InheritExpireDate && current.PackageEndDate != nil {
		endDate = *current.PackageEndDate
	}
	if packageChanged && options.InheritTraffic && current.PackageStartDate != nil {
		startDate = *current.PackageStartDate
	}
	candidate := current
	candidate.PackageID = packageID
	candidate.PackageStartDate = &startDate
	candidate.PackageEndDate = &endDate
	candidate.IsReset = isReset
	candidate.ResetDay = resetDay
	if packageChanged {
		candidate.TrafficWarned80 = false
		candidate.OverLimitEnforced = false
		candidate.TrafficLimitOverride = nil
		now := time.Now()
		candidate.LastResetAt = &now
	}

	oldNodes := oldPackage.Nodes
	if !packageChanged {
		// A same-package save is also a consistency repair for missing clients.
		oldNodes = nil
	}
	updater := &PackageUpdateHandler{repo: h.repo, remoteManage: h.remoteManage, pusher: h.pusher}
	newUser := userForPackageAssignment(base, candidate)
	if err := updater.syncPackageUserNodesTransactionally(ctx, []storage.User{newUser}, oldNodes, newPackage.Nodes); err != nil {
		return nil, fmt.Errorf("套餐配置未能在全部服务器生效: %w", err)
	}
	if err := h.repo.UpdateUserPackageAssignment(ctx, candidate); err != nil {
		oldUser := userForPackageAssignment(base, current)
		rollbackErr := updater.syncPackageUserNodesTransactionally(context.WithoutCancel(ctx), []storage.User{oldUser}, newPackage.Nodes, oldPackage.Nodes)
		return nil, errors.Join(err, rollbackErr)
	}
	if packageChanged && !options.InheritTraffic {
		if err := h.repo.ResetPackageAssignmentTrafficCycleAt(ctx, candidate.ID, time.Now()); err != nil {
			return []string{"套餐已切换，但流量周期重置失败: " + err.Error()}, nil
		}
	}
	if h.pusher != nil {
		go h.pusher.PushToAllServersForUser(context.Background(), current.Username)
	}
	return nil, nil
}
...
func inboundCredLock(username string, assignmentID, serverID int64, inboundTag string) *sync.Mutex {
	key := fmt.Sprintf("%s|%d|%d|%s", username, assignmentID, serverID, inboundTag)
...
func getOrCreateInboundCredential(ctx context.Context, repo *storage.TrafficRepository,
	...
	lock := inboundCredLock(user.Username, user.PackageAssignmentID, serverID, inboundTag)
	lock.Lock()
	defer lock.Unlock()
	...
	if existing, _ := getProvisionedInboundConfig(ctx, repo, user, serverID, inboundTag); existing != nil && existing.Protocol == protocol {
	...
	if reuse := extractClientByEmail(settings, email); reuse != nil {
		...
		credJSON := string(b)
		_ = saveProvisionedInboundConfig(ctx, repo, storage.UserInboundConfig{
			AssignmentID: user.PackageAssignmentID, Username: user.Username, ServerID: serverID, InboundTag: inboundTag,
...
func generateCredential(protocol string, user storage.User, method, inboundTag string) (map[string]interface{}, string, error) {
	...
	case "snell":
		...
		cred["level"] = 0
	case "mieru":
		// mieru 按 username 认人。同一父入站挂多个子节点时，必须一人一号，
		// 不能再用面板用户名/套餐号，否则只有最先连上的那个能用。
		accountName := email
		if strings.TrimSpace(accountName) == "" {
			accountName = user.Username
			if inboundTag != "" {
				accountName = user.Username + "__" + inboundTag
			}
			if user.PackageAssignmentID > 0 {
				accountName = fmt.Sprintf("%s-p%d", accountName, user.PackageAssignmentID)
			}
		}
		cred["username"] = accountName
		cred["password"] = uuid.New().String()
		cred["email"] = email
		cred["level"] = 0
	case "hysteria":
	...
	case "shadowsocks":
		if isShadowsocks2022Method(method) {
			...
			// Legacy Shadowsocks multi-user credentials are independent from the
			// inbound-level password. Xray also requires each user to carry its
			// cipher method; the parent method/password are ignored in this mode.
			cred["method"] = strings.TrimSpace(method)
			cred["password"] = uuid.New().String()
		}
		cred["email"] = email
		cred["level"] = 0
	case "socks", "http":
		accountName := user.Username
		if user.PackageAssignmentID > 0 {
			accountName = fmt.Sprintf("%s-p%d", user.Username, user.PackageAssignmentID)
		}
		cred["user"] = accountName
