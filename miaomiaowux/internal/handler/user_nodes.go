...
func (h *UserNodesHandler) HandleListNodes(w http.ResponseWriter, r *http.Request) {
	...
	if user.PackageID == 0 {
		writeJSONError(w, http.StatusNotFound, "未绑定套餐")
		return
	}

	pkg, err := h.repo.GetPackage(ctx, user.PackageID)
	if err != nil {
		writeJSONError(w, http.StatusNotFound, "套餐不存在")
		return
	}
	effectiveNodes, err := effectivePackageNodeIDs(ctx, h.repo, pkg.Nodes, pkg.NodesConfigured)
	if err != nil {
		writeJSONError(w, http.StatusInternalServerError, "获取套餐节点失败")
		return
	}

	var nodes []userNodeInfo
	for _, nodeID := range effectiveNodes {
...
func (h *UserNodesHandler) validateNodeAccess(ctx context.Context, username string, nodeID int64) (storage.Node, int64, error) {
	...
	if user.PackageID == 0 {
		return storage.Node{}, 0, fmt.Errorf("未绑定套餐")
	}

	pkg, err := h.repo.GetPackage(ctx, user.PackageID)
	if err != nil {
		return storage.Node{}, 0, fmt.Errorf("套餐不存在")
	}
	effectiveNodes, err := effectivePackageNodeIDs(ctx, h.repo, pkg.Nodes, pkg.NodesConfigured)
	if err != nil {
		return storage.Node{}, 0, fmt.Errorf("获取套餐节点失败")
	}
	...
	if !found {
		return storage.Node{}, 0, fmt.Errorf("节点不在您的套餐内")
	}
