...
func (h *UserRoutedOutboundHandler) create(w http.ResponseWriter, r *http.Request, username string) {
	...
	userCred, _, err := generateRoutedClientCred(parent.Protocol, protocolMethodFromNode(parent), userEmail)
	if err != nil {
...
func (h *UserRoutedOutboundHandler) delete(w http.ResponseWriter, r *http.Request, username string) {
	...
	if err != nil {
		// agent 不可达也尝试清 DB,避免僵尸记录
		log.Printf("[UserRoutedOutbound] delete: resolve server failed (continue clean DB): %v", err)
	} else {
		// best-effort:逐步拆,出错只记录
...
func deleteUserPrivateRoutedAll(ctx context.Context, rm *RemoteManageHandler, repo *storage.TrafficRepository, username string) {
	nodes, err := repo.ListUserRoutedOutbounds(ctx, username)
	if err != nil {
		log.Printf("[DeleteUserRouted] list %s failed: %v", username, err)
		return
	}
	for _, n := range nodes {
		serverID, err := resolveServerIDByNameRepo(ctx, repo, n.OriginalServer)
		if err == nil && rm != nil {
			...
			rm.forwardToRemoteServer(ctx, serverID, "POST", "/api/child/outbounds", rmOutBody)
			sa, _ := repo.GetUserSubaccount(ctx, n.ID, username)
			if sa != nil {
				removeClientFromInbound(ctx, rm, serverID, n.InboundTag, sa.Email)
			}
		...
		if err := repo.DeleteRoutedNode(ctx, n.ID); err != nil {
			log.Printf("[DeleteUserRouted] DB delete node %d failed (continue): %v", n.ID, err)
		}
	}
}
