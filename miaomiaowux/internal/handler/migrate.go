...
func (h *MigrateHandler) FetchMmwBackup(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSONError(w, http.StatusBadGateway, fmt.Sprintf("登录MEO失败: %v", err))
		return
	}
...
// 用户上传MEO后台备份 zip(同 fetch 接口拿到的格式)。
...
func (h *MigrateHandler) UploadMmwBackup(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.HasSuffix(strings.ToLower(header.Filename), ".zip") {
		writeJSONError(w, http.StatusBadRequest, "请上传MEO后台导出的 zip 备份")
		return
	}
...
func mmwLogin(ctx context.Context, client *http.Client, baseURL, username, password, totp string) (string, error) {
	...
	if tok == "" {
		return "", errors.New("登录响应中缺少 token (可能不是MEO实例?)")
	}
	return tok, nil
}
...
func extractMmwBackup(zipPath, outDBPath, outSubsDir string) (int64, int, error) {
	...
	if !dbFound {
		return 0, 0, errors.New("zip 中没有 data/*.db,该归档可能不是有效的MEO备份")
	}
...
func (h *MigrateHandler) ImportMmw(w http.ResponseWriter, r *http.Request) {
	...
	// 2.5 MEO备份可能带入它自己的 admin → 系统出现多个管理员。
...
//
// 用途:从MEO迁移过来,被MEO用 multi-conf 方式管理的 xray 服务器,
...
//
// 为什么需要:
//   - MEO时代 xray inbound 一般只配一个 client,没设 email
