...
// 节点测速(PRO speed_test,Phase 1:主控本机用 mihomo 内核测单节点)。
...
func NewSpeedTestHandler(repo *storage.TrafficRepository, lic *license.Manager) *SpeedTestHandler {
	return &SpeedTestHandler{repo: repo, license: lic}
}

// SetTesterWS 注入家用测速端 WS handler,启用"经家用测速端测速"。
func (h *SpeedTestHandler) SetTesterWS(t *SpeedTesterWSHandler) { h.testerWS = t }

func (h *SpeedTestHandler) proEnabled() bool {
	if h.license == nil {
		return false
	}
	return h.license.HasFeature(featureSpeedTest)
}

func (h *SpeedTestHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if !h.proEnabled() {
		writeError(w, http.StatusForbidden, errors.New("本地节点测速功能不可用"))
		return
	}
	switch {
