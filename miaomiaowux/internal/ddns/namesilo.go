...
func (p *namesiloProvider) ReconcileRecordSet(ctx context.Context, fqdn string, recordType string, desiredContents []string, ttl int) error {
	zone, sub, err := SplitFQDN(fqdn)
	if err != nil {
		return fmt.Errorf("split fqdn: %w", err)
	}
	...
	for _, rec := range listResp.Reply.ResourceRecord {
		if rec.Host == expectHost && strings.EqualFold(rec.Type, recordType) {
			existing[rec.Value] = true
			idByContent[rec.Value] = rec.RecordID
		}
	}
	...
	for content, id := range idByContent {
		...
		delResp, err := p.client.DnsDeleteRecord(ctx, &namesilo.DnsDeleteRecordParams{Domain: zone, ID: id})
		if err != nil {
			return fmt.Errorf("delete record: %w", err)
		}
		if delResp == nil || delResp.Reply.Code != "300" {
			return fmt.Errorf("namesilo delete reply: code=%s detail=%s", delResp.Reply.Code, delResp.Reply.Detail)
		}
	}
	return nil
}
