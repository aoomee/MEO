...
func (m *Manager) resolveProviderID(ctx context.Context, server *storage.RemoteServer, fqdn string) (int64, error) {
	...
	return m.resolveProviderForDomain(ctx, fqdn)
}

// resolveProviderForDomain 不依赖具体 server:先按证书绑定的服务商,再遍历所有 DNS 服务商用
// 「能管辖该域名(CanManage 只读探测)」的第一个。供入口组 DNS 编排(无单一 server)与 DDNS 自动模式共用。
func (m *Manager) resolveProviderForDomain(ctx context.Context, fqdn string) (int64, error) {
	// 按证书
	if cert, err := m.repo.FindCertificateForDomain(ctx, fqdn); err == nil && cert != nil && cert.DNSProviderID > 0 {
		return cert.DNSProviderID, nil
	}
	// 遍历 DNS 服务商,只读探测谁能管辖这个域名(不写任何记录)
...
// ReconcileGroupDNS 把入口组域名 fqdn 的 A 记录集调成恰好 desiredIPs(健康成员的对外 IPv4)。
// 供转发管理入口组 DNS 负载均衡:健康成员多 → 多条 A;成员掉线 → 摘除对应记录;全掉线 → 清空。
// providerID>0 用指定服务商;==0 时按域名自动解析服务商。desiredIPs 应为调用方已校验的 IPv4 字符串。
func (m *Manager) ReconcileGroupDNS(ctx context.Context, providerID int64, fqdn string, desiredIPs []string) error {
	fqdn = strings.TrimSpace(fqdn)
	if fqdn == "" {
		return fmt.Errorf("ReconcileGroupDNS: fqdn 为空")
	}
	if providerID <= 0 {
		id, err := m.resolveProviderForDomain(ctx, fqdn)
		if err != nil {
			return err
		}
		providerID = id
	...
	if err := provider.ReconcileRecordSet(ctx, fqdn, "A", desiredIPs, 0); err != nil {
		return fmt.Errorf("provider=%s ReconcileRecordSet(%s): %w", providerType, fqdn, err)
	}
	return nil
}
