...
func (p *dnspodProvider) ReconcileRecordSet(ctx context.Context, fqdn string, recordType string, desiredContents []string, ttl int) error {
	zone, sub, err := SplitFQDN(fqdn)
	if err != nil {
		return fmt.Errorf("split fqdn: %w", err)
	}
	...
	if err != nil {
		...
		for _, rec := range records {
			if rec.Name == sub && strings.EqualFold(rec.Type, recordType) {
				...
				idByContent[rec.Value] = rec.ID
			}
		}
	}
	toAdd, keep := diffRecordSet(desiredContents, existing)
	for _, content := range toAdd {
		if err := p.createRecord(zone, sub, recordType, content, ttl); err != nil {
			return err
		}
	}
	for content, id := range idByContent {
		...
		if _, err := p.client.Records.Delete(zone, id); err != nil {
