...
func (p *godaddyProvider) ReconcileRecordSet(ctx context.Context, fqdn string, recordType string, desiredContents []string, ttl int) error {
	zone, sub, err := SplitFQDN(fqdn)
	if err != nil {
		return fmt.Errorf("split fqdn: %w", err)
	}
	...
	// PUT /records/{type}/{name} 是整组 replace 语义,一次调用即 reconcile 到 desiredContents。
	// desiredContents 为空 → DELETE 该 (type,name) 全部记录(PUT 空数组 GoDaddy 会拒)。
	url := fmt.Sprintf("%s/domains/%s/records/%s/%s", p.baseURL, zone, recordType, sub)
	if len(desiredContents) == 0 {
		return p.doGoDaddy(ctx, http.MethodDelete, url, nil)
	}
	body := make([]godaddyRecord, 0, len(desiredContents))
	for _, content := range desiredContents {
		body = append(body, godaddyRecord{
			Type: recordType,
			Name: sub,
			Data: content,
			TTL:  ttl,
	...
	return p.doGoDaddy(ctx, http.MethodPut, url, body)
}

// doGoDaddy 发起 HTTP 请求(带鉴权头),2xx 视为成功,否则把官方错误码原样透出。
func (p *godaddyProvider) doGoDaddy(ctx context.Context, method, url string, body interface{}) error {
