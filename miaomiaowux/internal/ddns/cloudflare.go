...
func (p *cloudflareProvider) findRecord(ctx context.Context, zoneID string, fqdn string, recordType string) (*cfDNSRecord, error) {
	records, err := p.listRecords(ctx, zoneID, fqdn, recordType)
	if err != nil {
		return nil, err
	}
	if len(records) == 0 {
...
func (p *cloudflareProvider) listRecords(ctx context.Context, zoneID string, fqdn string, recordType string) ([]cfDNSRecord, error) {
...
func (p *cloudflareProvider) ReconcileRecordSet(ctx context.Context, fqdn string, recordType string, desiredContents []string, ttl int) error {
	zone, _, err := SplitFQDN(fqdn)
	if err != nil {
		return fmt.Errorf("split fqdn: %w", err)
	}
	...
	for _, rec := range records {
		existing[rec.Content] = true
	}
	toAdd, keep := diffRecordSet(desiredContents, existing)
	for _, content := range toAdd {
		if err := p.createRecord(ctx, zoneID, recordType, fqdn, content, ttl); err != nil {
			return err
		}
	}
	for _, rec := range records {
