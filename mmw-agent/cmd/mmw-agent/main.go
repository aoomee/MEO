...
func main() {
	...
	// 自托管构建不依赖独立的 Agent Guard 进程。
	// mmwx-guardd-agent 守护进程,Guard 不可用即拒绝启动。现在 Agent 单独运行,
	// guardclient 为本地桩(恒健康/已授权),无需任何外部 Guard。
	...
	// 转发规则持久化 + 启动重建(对齐官方 0.6.6:agent 重启/升级不再断转发)。
	// 落盘文件放配置文件同目录;没有配置文件时退回日志目录。
	if fwdDir := forwardPersistDir(cfgFile, cfg.LogPath); fwdDir != "" {
		manageHandler.SetForwardPersistPath(filepath.Join(fwdDir, "forward-rules.json"))
	}
	manageHandler.SetXrayAccessLogPath(xrayAccessLog) // 内嵌模式 service=xray 读它
	// guardclient 为本地兼容实现，运行时与升级仍统一读取 Enforced()。
	agentGuard := guardclient.NewFromEnv()
	manageHandler.SetActionGuard(agentGuard)
	if guardclient.Enforced() {
		log.Fatal("[Main] Enforced()=true 需要独立 Guard，本构建不支持")
	}
	log.Printf("[Main] Agent Guard 未强制(Enforced=false)，跳过 mmwx-guardd-agent")
...
// forwardPersistDir 选转发规则落盘目录:优先配置文件所在目录,其次日志目录,
// 目录须已存在(或可创建)。都拿不到则返回空,不做持久化。
func forwardPersistDir(cfgFile, logPath string) string {
	for _, cand := range []string{cfgFile, logPath} {
		if cand == "" {
			continue
		}
